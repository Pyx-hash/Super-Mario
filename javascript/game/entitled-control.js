...
function createGoombas() {
    ...
    for (i = 0; i < Math.trunc(worldWidth / 960); i++) {
        ...
        this.physices.add.collider(goomba, platformPieces);
     ...
     this.physics.add.collider(this.goombasGroup.getChildren(), this.failProtectionGroup.getChildren());
     this.physics.add.collider(this.goombasGroup.getChildren(), this.finalTrigger);
     setInterval(clearGoombas.call(this), 250);
}

function checkGoombasCollision(player, goomba) {
    if (goomba.dead)
    return;
...
function clearGoombas() {
    ...
    for (let i = 0; i < goombas.length; i++) {
        if (goombas[i].body.velocity.x == 0 || (goombas[i].body.velocity.x > 0 && goombas[i].body.velocity.x != goombasVelocityX)
        || (goombas[i].body.velocity.x < 0 && goombas[i].body.velocity.x != -goombasVelocityX)) {
